...
import com.gestionetudiants.dao.RTDAO;
...
public class RTServlet extends HttpServlet {
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        System.out.println("🔍 Servlet RTServlet exécuté !");

        // Récupération des étudiants de HEC
        List<Etudiants> etudiants = RTDAO.getEtudiants();
        System.out.println("Nombre d'étudiants trouvés : " + etudiants.size());

        // Envoi des étudiants à la page JSP
        request.setAttribute("etudiants", etudiants);
        request.getRequestDispatcher("RT.jsp").forward(request, response);
    }
}
